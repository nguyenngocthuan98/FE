...
var quantity_add = function (event, max = 0) {
    if (max == 0) {
        ...
        else control.val(parseInt(control.val()) + 1);
    } else {
        var control = $(event.target).parents(".custom-number").find("input");
        if (control.val() < 1) {
            control.val(1);
...
function convertToSlug(str) {
    ...
    str = str.toLowerCase();
    var from =
        "äàáảãạăằắẳẵặâầấẩẫậđèéẻẽẹêềếểễệëìíỉĩịòóỏõọôồốổỗộơờớởỡợưỳýỷỹỵùúủũụưừứửữựïîöñç·/_,:;";
    var to =
        "aaaaaaaaaaaaaaaaaadeeeeeeeeeeeeiiiiiooooooooooooooooouyyyyyuuuuuuuuuuuiionc------";
